function [STA,STC,RawMu,RawCov] = warnerSTC(Stim, sp, t_SampRatio, nkt, maxsize)
...
% MaxSize [1 x 1]   - max # of floats to store while computing cov (optional)
...
% $Id$
%
% Modified by Chris Warner May 2019 to allow for stimulus and spikes to
% happen at different time frames.


verbose = 1; % To make plots and troubleshoot code. 
...
if (nsp ~= slen*t_SampRatio)
    fprintf(1, 'simpleSTC: converting spike times to counts\n');
    sp = hist(sp,1:slen*t_SampRatio)';
end
...
nspnds = sum(sp~=0);    % number of spikes

% ---------------------------------------------------
% 1. Compute the spike-triggered STA and STC
 
Msz = nspnds*swid*nkt;  % size of design matrix for spiking stimuli (#spikes*spatialDims*#timeBins)
iisp = find(sp);        % spike/activation times
splen = length(iisp);   % number of spikes


if verbose
    disp([' num spikes (nspnds) = ', num2str(nspnds)] )
    disp([' num pixels (swid) = ', num2str(swid)] )
    disp([' num stim time bins (slen) = ', num2str(slen)] )
    disp([' num resp time steps in design matrix (nkt) = ', num2str(nkt)] )
    disp([' size of design matrix for each spike (Msz) = ', num2str(Msz)] )
    disp([' num spikes (splen) = ', num2str(splen)] )
end


if Msz < maxsize  % Compute in one chunk if small enough
    SS = makeStimRows(Stim, nkt, iisp, t_SampRatio); % SS is the 'design matrix'
    ...
    i0 = 1;
    imx = chunksize;
    SS = makeStimRows(Stim,nkt,iisp(i0:imx), t_SampRatio);
...
if nargout > 2  
    ...
    if Msz < maxsize  % Check if stimulus is small enough to do in one chunk
        ...
        SS = makeStimRows(Stim,nkt,'valid', t_SampRatio); % Convert stimulus to matrix where each row is one stim
        ...
        SS = makeStimRows(Stim(1:chunksize+nkt-1,:),nkt,'valid', t_SampRatio);  % convert stimulus to "full" version
