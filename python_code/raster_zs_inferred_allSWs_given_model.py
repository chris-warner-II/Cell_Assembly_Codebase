...
def raster_zs_inferred_allSWs_given_model(args): # y=DEFAULTS["y"], ):  #
	...
	rasterZ_data_save_file = str(data_save_dir + init_dir + model_dir + model_file.replace('LearnedModel_',str('rasterZ_allSWs_'+maxTrialTag+'_') ))
	...
	if fExists:
		...
		# if True:
		# 	SWs_xVal = data['SWs_test'] 	# if running on the rand0 model
		# else:
		# 	SWs_xVal = data['SWs_train']	# if running on the rand0B model
		...
		if whichGLM=='real':
			...
			fname_SWs = str( SW_extracted_Dir + 'GLMsim_' + whichCells + '_' + whichPop + '_' + whichGLM + '_' + stim + '_' + str(msBins) + 'msBins.npz' )
		...
		if np.isnan(maxRasTrials):
			numTrials = len(SWs)
		else:
			numTrials=int(maxRasTrials)
			print('NumTrials limited to  ',numTrials,'. There are actually ',len(SWs))
		...
		# # # Infer Y and Z for test data for cross validation.
		# print('Inferring Z,Y,pjoint for X-val Data - "test" data - with model post-EM-learning')
		# t0 = time.time()
		...
		# xxx
		# Z_inferred_allSWs, pyiEq1_gvnZ_allSWs, pj_inferred_allSWs, cond_inferred_allSWs = rc.inferZ_allSWs( SWs_xVal, \
		# 	SWtimes_xVal, CAs, numTrials, minTms, maxTms, qp, rip, riap, flg_include_Zeq0_infer, yLo, yHi, verbose=False )
		# t1 = time.time()										
		# print('Done Inferring Z,Y,pjoint for X-val Data - "test" data - with model post-EM-learning: time = ',t1-t0)
...
if __name__ == "__main__":
	...
	raster_zs_inferred_allSWs_given_model(args) #args.x, args.y)  
