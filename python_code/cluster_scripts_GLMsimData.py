...
cell_types = ['allCells'] 
	...
	# subpop + ind + offBT
	# subpop + cpl + offBT
	# subpop + ind + offBT_offBS
	# subpop + cpl + offBT_offBS
	#
	# fullpop + ind + offBT
	# fullpop + ind + offBT_offBS_onBT
	# fullpop + ind + offBT



whichCells = ['offBT','offBT_offBS','offBT_onBT'] # NOTE: should match order and number in cellSubTypeCombinations
whichGLMs = ['ind'] #['ind','cpl']
whichPops = ['fullpop'] #['subpop','fullpop']

# a list of lists. Each internal list is a combination of cell sub types to consider as a group to find Cell Assemblies within them.
cellSubTypeCombinations = [ ['offBriskTransient'], ['offBriskTransient','offBriskSustained'], ['offBriskTransient','onBriskTransient']  ]



what_to_run = 'rasG' 	# (1). 'pgmG' - Learn model on GLM simulation data. 
						# (2). 'rasG' - To infer Z activity for all spike words post model learning.

mem = 7500 # amount of memory (in MB) to allocate per CPU for each job submitted to cluster. 
			# pgmR needs more memory, (3500) when running for 500k EM samples.
			# rasZ needs more memory (7500) when running for all spike words also for N=98
...
SW_bins = [2,1]#, 2,1,0] 					# ms. Build up spikewords from groups of cells in the same trial that fire within SW_bins of eachother.
...
maxRasTrials = np.nan # np.nan to use all trials when inferring all spikewords post learning. Otherwise use a small number here.
			...
			for yHi in yHi_Vals:
				...
				for yMinSW in yMinSWs:
					...
					for cell_type in cell_types:
						#
						for whichGLM in whichGLMs:
							#
							for whichPop in whichPops:
								#
								for jj, cellSubTypes in enumerate(cellSubTypeCombinations):
									whichCell = whichCells[jj] 
									#
															...
															for flg_EgalitarianPrior in flg_EgalitarianPriorS:
																...
																for sample_longSWs_1st in sample_longSWs_1stS:
																	...
																	for maxSamps in maxSampsS:
																		...
																		if what_to_run is 'pgmG':
																			ss.write_sbatch_script_pgmCA_GLMsimData(script_path=script_path, output_path=output_path, error_path=error_path, 
																				whichClust=whichClust, n_cores=n_cores, mem=mem, time=time, job_name=fname, whichCell=whichCell, whichGLM=whichGLM, whichPop=whichPop,
																				cell_type=cell_type, cellSubTypes=cellSubTypes, stim=stim, num_test_samps_4xVal=num_test_samps_4xVal, rand=rand, SW_bin=SW_bin, 
																		...
																		if what_to_run is 'rasG':
																			#
																			# Infer Z's for all Spike Words (in train data set if train_2nd_model=False; in test dataset if train_2nd_model=True)
																			ss.write_sbatch_script_rasterZ_GLMsimData(script_path=script_path, output_path=output_path, error_path=error_path, 
																				whichClust=whichClust, n_cores=n_cores, mem=mem, time=time, job_name=fname, cell_type=cellSubTypes,
																				whichCells=whichCell, whichGLM=whichGLM, whichPop=whichPop, stim=stim, num_test_samps_4xVal=num_test_samps_4xVal, 
																				rand=rand, SW_bin=SW_bin, yLo=yLo, yHi=yHi, yMinSW=yMinSW, params_init=params_init, sigQ_init=sigQ_init, 
																				sigPi_init=sigPi_init, sigPia_init=sigPia_init, overcomp=overcomp, Z_hot=Z_hot, C_noise_ri=C_noise_ri, 
																				C_noise_ria=C_noise_ria, learning_rate=learning_rate, lRateScale=lRateScale, maxTms=maxTms, minTms=minTms, 
																				pct_xVal_train=pct_xVal_train, flg_EgalitarianPrior=flg_EgalitarianPrior, train_2nd_model=train_2nd_model, 
																				flg_include_Zeq0_infer=flg_include_Zeq0_infer, sample_longSWs_1st=sample_longSWs_1st, maxSamps=maxSamps, 
																				maxRasTrials=maxRasTrials, flg_checkNPZvars=flg_checkNPZvars, verbose_EM=verbose_EM)
																			...
																			if train_2nd_model: 
																				...
																				ss.write_sbatch_script_rasterZ_GLMsimData(script_path=script_path, output_path=output_path, error_path=error_path, 
																					...
																					sigPi_init=sigPi_init, sigPia_init=sigPia_init, overcomp=overcomp, Z_hot=Z_hot, C_noise_ri=C_noise_ri, 
																					C_noise_ria=C_noise_ria, learning_rate=learning_rate, lRateScale=lRateScale, maxTms=maxTms, minTms=minTms, 
																					pct_xVal_train=pct_xVal_train, flg_EgalitarianPrior=flg_EgalitarianPrior, train_2nd_model=False, 
																					flg_include_Zeq0_infer=flg_include_Zeq0_infer, sample_longSWs_1st=sample_longSWs_1st, maxSamps=maxSamps, 
																					maxRasTrials=maxRasTrials, flg_checkNPZvars=flg_checkNPZvars, verbose_EM=verbose_EM)
