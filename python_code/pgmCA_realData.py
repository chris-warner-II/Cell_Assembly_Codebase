...
def pgmCA_realData(args): # y=DEFAULTS["y"], ):  #
	...
	GField_STRFdata_File  = str(cell_type + '_STRF_fits_329cells.mat')
	...
	#
	if stim=='NatMov':
		spikesIn = spikes['spikesInCat']
		# not using 'triggers_catrep' but its in there.
	elif stim=='Wnoise':
		spikesIn = spikes['spikesInWn']
		# not using 'triggers_wnrep' but its in there.
	else:
		print(' Uuuh, which stim again? ')
	#
	...
	spikesInCellSubTypes = spikesIn[cellSubTypeIDs-1] # !!! This -1 converts from Matlab indexing to Python indexing
	...
	#print(cellRFs.keys())
	STRF_gaussD = cellRFs['STRF_GaussParamsD'] # % parameters are: [Amplitude, x0, sigmax, y0, sigmay, angle(in rad)]
	STRF_gaussL = cellRFs['STRF_GaussParamsL'] # L = Light Responses, D = Dark Responses, M = Mean Responses.
	STRF_timeD = cellRFs['STRF_TimeParamsD']
	STRF_timeL = cellRFs['STRF_TimeParamsL']
	del cellRFs
	...
	distRFs = pix2ret*sp.spatial.distance_matrix( np.array([STRF_gauss[:,1], STRF_gauss[:,3]]).T, np.array([STRF_gauss[:,1], STRF_gauss[:,3]]).T )
	#
	...
	fname_SWs = str( SW_extracted_Dir + str(cellSubTypes) + '_' + stim + '_' + str(1+2*SW_bin) + 'msBins.npz' )
	SWs, SWtimes = rc.extract_spikeWords(spikesInCellSubTypes, SW_bin, fname_SWs)
	#
	...
	#
	fname_EMlrn = str( EM_learning_Dir + initLearn_dir + dataModel_dir + 'LearnedModel_' + stim \
		+ '_' + str(num_SWs) + 'SWs_' + str(pct_xVal_train).replace('.','pt') + 'trn_' \
	...
	# if os.path.isfile(fname_EMlrn):
	# 	if train_2nd_model:
	# 		if os.path.isfile(fname_EMlrnB):
	# 			print('The 2 file are there. Not remaking it.')
	# 			return
	# 	else:
	# 		print('The 1 file is there. Not remaking it.')
	# 		return
	...
	# #replace rip and ri_init with the spike rate of each cell.
	# if params_init == 'DiagonalPia_ratePiFixed':
	# 	# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
...
if __name__ == "__main__":
	...
	#
	# cell_type
	parser.add_argument('-ct', '--cell_type', dest='cell_type', type=str, default='allCells', #default=DEFAULTS["cell_type"],
		help='What cell type to process from GField retina data. ex.(allCells)')
	# cellSubTypes
	parser.add_argument('-cst', '--cellSubTypes', dest='cellSubTypes', type=str, default=['offBriskTransient','offBriskSustained'], #default=DEFAULTS["cell_type"],
		help='What cell type to process from GField retina data. ex.(allCells)')
	# stim
