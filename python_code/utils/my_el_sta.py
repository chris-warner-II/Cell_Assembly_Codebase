...
def spike_triggered_average(signal, spiketrains, window):
    ...
    # Editting this to work in a less fucking moronic and memory-hoggish way.
    ...
    # THIS IS JUST FUCKING DUMB AND WRONG !!!
    ...
    #     print('template=',template,' and num_signals=',num_signals)
    #     spiketrains = []
    ...
    for i in range(len(spiketrains)):
        if spiketrains[i].t_start < signal.t_start:
            raise ValueError( "The spiketrain indexed by %i starts earlier than the analog signal." % i)
        if spiketrains[i].t_stop > signal.t_stop:
            raise ValueError( "The spiketrain indexed by %i stops later than the analog signal." % i)
    ...
    window_bins = int(np.ceil(((window_stoptime - window_starttime) * signal.sampling_rate).simplified))
    #
    # result_sta: array containing finally the spike-triggered averaged signal
    result_sta = AnalogSignal(np.zeros((window_bins, num_signals)), sampling_rate=signal.sampling_rate, units=signal.units)
    #
    ...
    for i in range(num_signals):
        ...
        for spiketime in spiketrains[0]:
            ...
            if (spiketime + window_starttime >= signal.t_start and spiketime + window_stoptime <= signal.t_stop):
                ...
                startbin = int(np.floor(((spiketime + window_starttime - signal.t_start) * signal.sampling_rate).simplified))
