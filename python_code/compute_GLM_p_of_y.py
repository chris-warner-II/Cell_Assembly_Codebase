...
for i,ct in enumerate(cell_types): # Loop through cell type combinations
	...
	for stim in stims: # Loop through stimulus
		...
		for SW_bin in SW_bins: # Loop through binning of spike trains
			...
			for tr in range(nTrials): # Loop through trials
				...
				for t,sw in enumerate(SWs[tr]): # Loop through spike words in a trial
					...
					#
						...
						pOff = pyiEq0[out,tBin_indx-bBck:tBin_indx+bFwd].prod(axis=1)		# prob of each cell that is off to be off
						pOn = 1- pyiEq0[inn,tBin_indx-bBck:tBin_indx+bFwd].prod(axis=1)		# prob of all cell that is on to be on
						pY = pOn.prod()*pOff.prod()
